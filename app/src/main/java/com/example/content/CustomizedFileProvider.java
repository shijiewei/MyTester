 ...
 * </p>
 */
public class CustomizedFileProvider extends ContentProvider {
     ...
     *
     * @param context A {@link Context} for the current component.
     * @param authority The authority of a {@link CustomizedFileProvider} defined in a
...
//                            "Failed to parse " + META_DATA_FILE_PROVIDER_PATHS + " meta-data", e);
//                }
				strat = parsePathStrategyNonXml(context, authority);
        ...
        while ((type = in.next()) != END_DOCUMENT) {
            if (type == START_TAG) {
                ...
                if (TAG_ROOT_PATH.equals(tag)) {
                    ...
                    File[] externalFilesDirs = Utils.getExternalFilesDirs(context, null);
                    if (externalFilesDirs.length > 0) {
                        target = externalFilesDirs[0];
                    }
                } else if (TAG_EXTERNAL_CACHE.equals(tag)) {
                    File[] externalCacheDirs = Utils.getExternalCacheDirs(context);
	 ...
	 * @see #getPathStrategy(Context, String)
	 */
	private static PathStrategy parsePathStrategyNonXml(Context context, String authority) {
		final SimplePathStrategy strat = new SimplePathStrategy(authority);

		String name = Const.SHARABLE_PATH_ALIAS;
		String path = Const.SHARABLE_PATH;
		File target = null;
		File[] externalFilesDirs = Utils.getExternalFilesDirs(context, null);
		if (externalFilesDirs.length > 0) {
			target = externalFilesDirs[0];
